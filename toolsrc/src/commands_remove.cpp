...
namespace vcpkg::Commands::Remove
{
    ...
    void perform_and_exit(const vcpkg_cmd_arguments& args, const vcpkg_paths& paths, const triplet& default_target_triplet)
    {
        ...
        if (options.find(OPTION_OUTDATED) != options.cend())
        {
            ...
            status_db = database_load_check(paths);
            const std::vector<outdated_package> outdated_packages = Update::find_outdated_packages(paths, status_db);
            for (auto&& p : outdated_packages)
            {
        ...
        else
        {
            ...
            specs = Input::check_and_get_package_specs(args.command_arguments, default_target_triplet, example);
            Input::check_triplets(specs, paths);
            status_db = database_load_check(paths);
        }
